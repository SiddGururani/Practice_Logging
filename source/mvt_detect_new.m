function c_id = mvt_detect_new(filename, threshold, length)
...
save(['../distmatsnew/' char(filename) '.mat'], 'lag1', 'lag2', 'lag3');
% c_id = 1;

%% Load stored distance matrix. Use only if running grid search, or if distance matrices are stored.

% load(['../distmatsnew/' char(filename) '.mat']);
...
%% Erode and Dilate while looking for lines of specified length.
...
%% Compute average perimeter of boudaries detected in erode dilate.
% This is a heuristic quantification of similarity.
m1 = 0;
for i = 1:numel(b1)
...
%% Find maximum average similarity and output it as belonging to that song.
