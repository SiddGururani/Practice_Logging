...
if strcmp(ext,'.wav')
    % The next three lines have been edited from the original due to the
    % switch from wavread to audioread in MATLAB_2015b. RMW 11/12/2016.
    [f_audio,fs] = audioread(strcat(dirAbs,dirRel,wavfilename));
    audinfo = audioinfo(strcat(dirAbs,dirRel,wavfilename));
    nbits = audinfo.BitsPerSample;
else
